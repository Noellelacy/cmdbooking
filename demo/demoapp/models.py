...
class MultimediaEquipment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk and not self.added_by:
            # If this is a new equipment and no added_by is set,
            # try to get it from the current request
            from django.contrib.auth.models import User
            try:
...
class MultimediaEquipmentExtended(models.Model):
    CONDITION_CHOICES = (
        ('excellent', 'Excellent'),
        ('good', 'Good'),
        ('fair', 'Fair'),
    ...
    equipment = models.OneToOneField(MultimediaEquipment, on_delete=models.CASCADE)
...
class EquipmentUsageExtended(models.Model):
    ...
    def __str__(self):
        return f"{self.usage.user.username} - {self.usage.equipment.name}"
